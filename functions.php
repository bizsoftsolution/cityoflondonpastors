			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						echo ($eventNum > 0)?'<a href="javascript:;" onclick="getEvents(\''.$currentDate.'\');">'.$row['title'].'</a><br/>':'';
						//For Add Event
...
function getEvents($date = ''){
	...
	$result = $db->query("SELECT title,description FROM events WHERE date = '".$date."' AND status = 1");
	if($result->num_rows > 0){
		$eventListHTML = '<h2>Events on '.date("l, d M Y",strtotime($date)).'</h2>';
		$eventListHTML .= '<ul>';
		while($row = $result->fetch_assoc()){ 
            $eventListHTML .= '<li><b>'.$row['title'].'</b></li>';
            $eventListHTML .= '<p>'.$row['description'].'</p>';
        }
		$eventListHTML .= '</ul>';
	}
	echo $eventListHTML;
